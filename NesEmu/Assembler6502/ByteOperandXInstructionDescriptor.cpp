...
namespace Assembler6502 {
	...
	vector<uint8_t> ByteOperandXInstructionDescriptor::GetOperationCodes(const Labels& labels) {
		return vector<uint8_t> { GetOpCode(OpCodeEntry(GetInstructionType(), GetAddressMode(), Operator::X)), GetOperand(labels) };
	}
}